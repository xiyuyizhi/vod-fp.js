...
export default class TsToMp4 extends PipeLine {
    ...
    setUpPipeLine() {
        ...
        es = this.entryStream.pipe(new TsPacketParseStream()).pipe(new TsElementaryStream());
        es.pipe(new MetaDataStream()).pipe(remuxStream);
        this.audioStream = es.pipe(new AacStream())
            .pipe(remuxStream)
            .pipe(new AudioFragmentStream())
        this.videoStream = es.pipe(new AvcStream())
            .pipe(remuxStream)
