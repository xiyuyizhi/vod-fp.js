...
function _selectForceLevel(
  ...
  loadRate,
  maxCanLoadTime,
  currentLevelLoadedDelay,
  segment
) {
  return getState(ACTION.PLAYLIST.LEVELS)
    .map(levels => {
      ...
      for (let i = levelId - 1; i > 0; i--) {
        ...
        if (
          nextLevelLoadedDelay < maxCanLoadTime &&
          nextLevelLoadedDelay < currentLevelLoadedDelay
        ) {
          return i;
        }
      }
...
function _loadCheck(
  { getState, connect, dispatch, getConfig },
  startTs,
  segment
) {
  return () => {
    ...
    let segLoadToLong =
      (now - startTs) / 1000 > duration * LOAD_CHECK_REAL_DO_FACTOR;
      ...
      Maybe.of(
        curry((bufferLength, media, loadInfo) => {
          ...
          if (!total) {
            ...
            total = getState(ACTION.PLAYLIST.FIND_LEVEL, segment.levelId)
              .map(level => {
          ...
          if (
            fetchFinishDelay / bufferStarvationDelay >
            LOAD_ABORT_BUFFER_REFERENCE_FACTOR
          ) {
            logger.warn(
            ...
            let forcedLevel = connect(_selectForceLevel)(
              loadSpeed * 8 * SPEED_USAGE_FACTOR,
              bufferStarvationDelay,
              fetchFinishDelay,
              segment
            );
    ...
    curry(
      (estimator, bufferInfo, media, levels, segDuration, currentLevelId) => {
        ...
        let bufferStarvationDelay =
          bufferInfo.bufferLength / media.playbackRate;
        ...
        for (let i = maxLevel; i >= minLevel; i--) {
          ...
          let biterate = +levels[i - 1].bandwidth;
          if (
            biterate < ajustedBw &&
            (bufferStarvationDelay === 0 ||
              (biterate * segDuration) / ajustedBw < bufferStarvationDelay)
          ) {
            return i;
          }
        }
...
export { abrProcess, abrBootstrap, getNextABRLoadLevel };
