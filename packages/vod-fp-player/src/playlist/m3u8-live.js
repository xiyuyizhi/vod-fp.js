...
function _mergePlaylist({ getState, dispatch, connect }, levelId, newDetails) {
    ...
    getState(ACTION.PLAYLIST.FIND_LEVEL, levelId).map(level => {
            ...
            if (oldSegments[i + delta]) {
                ...
                if (!lastSegment) {
                    ...
                    if (lastSegment) {
                        ...
                        if (deltaCompareLastLevel < 0 && deltaCompareLastLevel > -2 && i != endSN - startSN) {
                            logger.warn(`can‘t find a matched segment for ${startSN + i}, startSN < lastLevel.startSN,continue loop`)
                            newStartSN = startSN + i + 1;
                            continue;
                        }
                        logger.warn(`can‘t find a matched segment for ${startSN + i},use total duration as sync start`)
        ...
        logger.log(`slide position:${first.start} , `, `segments rangs: [${first.start},${last.end}]`)
    });
    return noNews;
}
...
function _caclLiveSyncPosition({ getState, getConfig }) {
    return Maybe.of(curry((segs, duration) => {
        let segsTotalDuration = segs.map(prop('duration')).reduce((all, c) => {
            all += c;
            return all;
        }, 0)
        return duration - segsTotalDuration * getConfig(ACTION.CONFIG.LIVE_LATENCY_FACTOR);
    }))
...
function checkSyncLivePosition({ getState, connect }, media, bufferEnd) {
    ...
    Maybe.of(curry((live, slidePosition) => {
        //检测当前播放时间点是否脱离直播位置
        if (bufferEnd < slidePosition) {
            logger.log('bufferEnd:', bufferEnd, 'slidePosition:', slidePosition, 'end:', media.duration);
            connect(_caclLiveSyncPosition).map(liveSyncPostion => {
                media.currentTime = liveSyncPostion;
                logger.log('脱离直播点,sync position', media.currentTime)
            })
        }
    }))
        .ap(getState(ACTION.PLAYLIST.IS_LIVE))
...
export { bootStrapFlushPlaylist, checkSyncLivePosition }